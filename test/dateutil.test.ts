...
tap.test('formatDateTime', async (t) => {
  t.test('formats date with default format', async (t) => {
    ...
    t.equal(
      formatDateTime(date, 'MMMM DD, YYYY hh:mm:ss a UTC'),
      'April 15, 2023 02:30:45 pm UTC',
    );
    t.end();
  });
  ...
  t.test('handles different months', async (t) => {
    ...
    t.equal(formatDateTime(date, 'MMMM DD, YYYY', 'UTC'), 'January 01, 2023');
    t.end();
  });
  ...
  t.test('formats time correctly', async (t) => {
    ...
    t.equal(formatDateTime(date, 'hh:mm:ss a', 'UTC'), '11:59:59 pm');
    t.end();
  });

  t.test('handles midnight correctly', async (t) => {
    const date = new Date('2023-04-15T00:00:00Z');
    t.equal(formatDateTime(date, 'hh:mm:ss a', 'UTC'), '12:00:00 am');
    t.end();
  });

  t.test('includes timezone information', async (t) => {
    const date = new Date('2023-04-15T14:30:45Z');
    t.equal(formatDateTime(date, 'UTC +00:00', 'UTC'), 'UTC +00:00');
    t.end();
  });

  t.test('uses current date when no date is provided', async (t) => {
    const now = new Date();
    const formatted = formatDateTime(new Date(), 'YYYY', 'UTC');
