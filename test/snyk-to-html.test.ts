...
test('test calling snyk-to-html from command line', (t) => {
  t.plan(1);
  exec(`node ${main} -i ./test/fixtures/multi-test-report.json`, { maxBuffer: 1024 * 1024 }, (err, stdout) => {
    if (err) {
      throw err;
    }
...
test('test snyk-to-html handles -a argument correctly', (t) => {
  ...
  exec(`node ${main} -i ./test/fixtures/test-report-with-remediation.json -a`, (err, stdout) => {
    ...
    const cleanTimestamp = rep => rep.replace(regex, '<p class="timestamp">TIMESTAMP</p>');
    const cleanedReport = cleanTimestamp(stdout);
    t.contains(cleanedReport, '<body class="test-remediation-section-projects">', 'should contain remediation section');
...
test('test snyk-to-html shows remediation with pins', (t) => {
  t.plan(3);
  exec(`node ${main} -i ./test/fixtures/test-report-with-pins-remediation.json --actionable-remediation`,
    (err, stdout) => {
    if (err) {
      throw err;
    }
    ...
    const cleanedReport = cleanTimestamp(stdout);
    t.contains(cleanedReport, '<body class="test-remediation-section-projects">', 'should contain remediation section');
    t.contains(cleanedReport, "<div class='remediation-card__pane shown test-remediation-pins'", 'should contain pins remediation section');
    t.matchSnapshot(cleanedReport, 'should be expected snapshot containing actionable remediation');
  });
});

test('test snyk-to-html shows remediation & summary', (t) => {
  t.plan(4);
  exec(`node ${main} -i ./test/fixtures/test-report-with-pins-remediation.json --actionable-remediation --summary`,
    (err, stdout) => {
    if (err) {
      throw err;
    }
...
test('test with remediations arg and data containing remediations object', (t) => {
  ...
  SnykToHtml.run(
      ...
      (report) => {
        ...
        t.contains(report, '<body class="test-remediation-section-projects">', 'should contain remediation section');
...
test('snyk-to-html run with remediation arg, some remediations and some vulns does not display vulns', (t) => {
  ...
  SnykToHtml.run(
      ...
      (report) => {
        ...
        t.match(report, 'remediation-card__block', 'should contain at least one remediation card');
        t.notMatch(report, 'class="card card--vuln"', 'should not display vulns');
        t.notMatch(report, '<h2>No known vulnerabilities detected.</h2>', 'should not display "no vulns" message');
      });
  });
...
test('snyk-to-html run with remediation arg, no remediations and some vulns displays vulns', (t) => {
  ...
  SnykToHtml.run(
      ...
      (report) => {
        // no actionable remediations displayed
        t.notMatch(report, '<div class="remediation-card">', 'should not contain remediation section');
        t.contains(report, '.card--vuln', 'should contain at least one vuln card');
      });
  });
...
test('snyk-to-html run with remediation arg, no remediations or vulns displays "no vulns" message', (t) => {
  ...
  SnykToHtml.run(
      ...
      (report) => {
        // no actionable remediations displayed
        t.notMatch(report, '<div class="remediation-card">', 'should not contain remediation section');
        t.notMatch(report, '.div class="card card--vuln"', 'should not contain vuln cards');
        t.contains(report, 'No known vulnerabilities detected', 'should display no vulns message if none present');
      });
});
...
test('displays metatable when metatable data present', (t) => {
  ...
  SnykToHtml.run(
    ...
    noSummary,
    (report) => {
      t.contains(report, '<table class="metatable">', 'should contain metatable');
    });
});
...
test('does not display metatable when no metatable data present', (t) => {
  ...
  SnykToHtml.run(
    ...
    noSummary,
    (report) => {
      t.doesNotHave(report, '<table class="metatable">', 'should not contain metatable');
    });
});
